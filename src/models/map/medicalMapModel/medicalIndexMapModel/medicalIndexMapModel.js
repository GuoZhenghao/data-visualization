...
export default {
    ...
    effects: {
        ...
        * getMedicalIndexMapLayerData({payload}, {call, put, select}) {
            let thisModel = yield select(state => state.medicalIndexMapModel);
            let result = yield call(medicalService.medicalIndexMapService, thisModel.param);
            // alert(JSON.stringify(result))
            if (result.data.result == true) {
                yield put({type: "getIndexMapLayerDataReducer", payload: result})
            } else {
    ...
    reducers: {
        ...
        getIndexMapLayerDataReducer(state, action) {
            if (state.medicalIndexMapClass != null) {
                state.indexMapLayerData = action.payload;
                state.medicalIndexMapClass.setMapLayerData(state.indexMapLayerData);
            }
            return {...state, ...action.payload};
        },
